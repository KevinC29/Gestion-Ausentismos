...
-- drop procedure actualizar_saldo_vacaciones_solicitud;
DELIMITER //
CREATE PROCEDURE actualizar_saldo_vacaciones_solicitud(
...
BEGIN
    ...
    SELECT sal_vac_dias_disponibles, sal_vac_dias_anio_actual, sal_vac_dias_un_anio, sal_vac_dias_dos_anios
    INTO p_sal_vac_dias_disponibles, p_sal_vac_dias_anio_actual, p_sal_vac_dias_un_anio, p_sal_vac_dias_dos_anios
    FROM Saldo_Vacacion
    WHERE pers_cedula = p_pers_cedula;
    
    IF p_sal_vac_dias_disponibles > 0 THEN
		-- Actualizar campos en Saldo_Vacacion
		...
		END IF;
	ELSE 
		SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'No cuenta con dias disponibles';
    END IF;
